 <?php 
    ...
    if(isset($_POST["login"])){
        if(empty($_POST["managerUsername"])||empty($_POST["managerPassword"])){
            $message = '<label>All fields are required</label>';
        }
        else{
            $query = "SELECT * FROM managertable WHERE managerUsername = :managerUsername AND managerPassword = :managerPassword";
            ...
            if($count>0){
                $_SESSION["managerUsername"] = $_POST["managerUsername"];
                header("location:dashboard.php");
            }
            else{
...
<html lang="en">
    <head>
        ...
        <title>Manager Login - Jeyml Mini Mart</title>
    </head>
                                    ...
                                    <div class="card-body">
                                        <form method="post">
                                            <div class="form-group">
                                                ...
                                                <input class="form-control py-4" id="inputUsername" type="text" name="managerUsername" placeholder="Enter username" required>
                                            </div>
                                            <div class="form-group">
                                                <label class="small mb-1" for="inputPassword">Password</label>
                                                <input class="form-control py-4" id="inputPassword" type="password" name="managerPassword" placeholder="Enter password" required>
                                            </div>
                                            <div class="form-group">
